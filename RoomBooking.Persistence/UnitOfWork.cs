...
namespace RoomBooking.Persistence
{
    public class UnitOfWork : IUnitOfWork
    {
        ...
        private async Task ValidateEntityAsync(object entity)
        {
            ...
            {
                var customerInDb = await _dbContext.Customers
                ...
                {
                    throw new ValidationException("Customer ist bereits in der Datenbank!");
                }

                if (!IbanChecker.CheckIban(customer.Iban))
                {
                    throw new ValidationException("Iban ist nicht gültig!");
                }                       
            }
